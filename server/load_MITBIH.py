...
def load_mit_db(DS, winL, winR, do_preprocess, maxRR, use_RR, norm_RR, compute_morph, db_path, reduced_DS, leads_flag):
    ...
    if os.path.isfile(features_labels_name):
        ...
        # ML-II
        if reduced_DS == False:
            DS1 = [101, 106, 108, 109, 112, 115, 118, 119, 200, 201, 202, 203, 205, 207, 208, 209, 217, 215, 219, 220, 223, 221, 222, 230, 233]
            DS2 = [105, 111, 113, 121, 212, 213, 210, 214, 228, 231, 232, 234]
        ...
        if 'wvlt' in compute_morph:
            ...
            for p in range(len(my_db.beat)): # outer 22 looping tiap data set
                for b in my_db.beat[p]: # looping untuk beat
                    f_wav_lead = np.empty([]) # inisialisi untuk tampung array
                    for s in range(2):
                        if leads_flag[s] == 1:
...
def load_signal(DS, winL, winR, do_preprocess):
    ...
    MITBIH_classes = ['N', 'L', 'R', 'e', 'j', 'A', 'a', 'J', 'S', 'V', 'E', 'F' ,'(VT' , '(AFIB']#, 'P', '/', 'f', 'u']
    AAMI_classes = []
    ...
    # add class to Normal, PVC, PAC , Atrial, Ventricular Tachycardia

    my_classes = []
    my_classes.append(['N', 'L' , 'R']) # 0 Normal
    my_classes.append(['V'])            # 1 PVC
    my_classes.append(['A'])            # 2 PAC
    my_classes.append(['(AFIB'])        # 3 Atrial Fib
    my_classes.append(['(VT'])          # 4 Ventricular Tachycardia
    #my_classes.append(['F'])            # 3
    ...
    for r in range(0, len(fRecords)):
       ...
       # print(RAW_signals)
        ...
        for a in annotations:
            ...
            # rhytm annotation
            if (len(aS) == 7): #check length of annotat
                classAnttd = aS[6]
            ...
            if (pos > size_RR_max and pos < (len(MLII) - size_RR_max)):
                ...
                #print("IDX-VAL" + str(value) + str(index))
