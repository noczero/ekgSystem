...
def eval_model(svm_model, features, labels, multi_mode, voting_strategy, output_path, C_value, gamma_value, DS):
    if multi_mode == 'ovo':
        ...
        if voting_strategy == 'ovo_voting':
            predict_ovo, counter    = ovo_voting(decision_ovo, 5) # 5 for total of class
...
def main(multi_mode='ovo', winL=90, winR=90, do_preprocess=True, use_weight_class=True, 
    maxRR=True, use_RR=True, norm_RR=True, compute_morph={''}, oversamp_method = '', pca_k = '', feature_selection = '', do_cross_val = '', C_value = 100, gamma_value = 0.0, reduced_DS = False, leads_flag = [1,0]):
    ...
    tr_features_scaled = tr_features #disable normalization
    ...
    if do_cross_val:
        ...
        perf_measures_path = create_svm_model_name('results/' + multi_mode, winL, winR, do_preprocess,
        ...
        model_svm_path = create_svm_model_name(model_svm_path, winL, winR, do_preprocess,
            maxRR, use_RR, norm_RR, compute_morph, use_weight_class, feature_selection,
            oversamp_method, leads_flag, reduced_DS, pca_k, '_') # file name
